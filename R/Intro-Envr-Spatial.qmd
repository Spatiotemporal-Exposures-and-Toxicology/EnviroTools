...
Environmental research relies on various types of spatial, temporal, and spatio-temporal data to accurately measure, predict, and model exposures. This vignette will introduce R packages that are equipped to handle these various types of data, and will demonstrate how to access, import, and analyze each data type.

The terminology in this vignette assumes users' basic understanding of spatial data, including point, polygon, and raster data types. Please see [Simple Features for R](https://r-spatial.github.io/sf/articles/sf1.html) for more information on point and polygon data types, and [Introduction to Raster Data](https://datacarpentry.org/organization-geospatial/01-intro-raster-data) for more information on raster data.

### 0.2 Objectives

This vignette is designed to assist users in developing spatial analysis skills including:
...
vignette_packages = c("dplyr", "ggplot2", "ggpubr", "sf",
...
library(terra); library(tidyterra); library(utils)
```

#### 0.4.1 `ggplot2`

Add text about reference resources for `ggplot2` package.

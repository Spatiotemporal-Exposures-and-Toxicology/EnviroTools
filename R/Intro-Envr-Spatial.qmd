...
min_max_monitors_id = c(min_monitors$Monitor.ID,
...
Wildfire smoke plume coverage data from the United States National Oceanic and Atmospheric Administration (NOAA) will be used to demonstrate **using** polygon data. This section will cover **using** polygon data with both the `sf` and `terra` packages, but the accessing, downloading, and unzipping of the data is the same for both packages.
...
Inspecting the file with `utils::unzip(list = TRUE)` returned the size of the file, but also the **name of the actual data file of interest** (`hms_smoke20230901.shp`). The desired data file can also be identified by using `list.files()`.
...
```{r, echo=FALSE, warning=TRUE, cache=FALSE, results='hide'}
################################################################################
smoke_sf = st_read("/Volumes/SET/Projects/EnviroTools/input/noaa/hms_smoke20230901.shp")
################################################################################
```
...
```{r}
smoke_sf = st_transform(smoke_sf, 
                        5070)
```

#### 2.1.5 Plot (single)

With the parameter of interest, coordinate reference system, and projection **checked and converted**, plot the wildfire smoke plume polygons with `ggplot2::ggplot()`.

```{r}
ggplot()+
  geom_sf(data = smoke_sf,
          aes(fill = Density))+
...
The plot, however, can be difficult to interpret for two main reasons. The first reason is that there are multiple polygons for each smoke density classification. Multiple polygon borders and overlapping polygons of the same smoke density can be confusing. To make plotting the polgyons more clear and easy to interpret, individual polygons can be combined according to the smoke plume density class.

::: callout-warning
**Overlapping polygons with the same smoke density classification can be conveying different information, including satellite travelling direction or time of collection. For the purposes of these exploratory analyses, the satellite travelling direction and time of collection will be ignored.**
:::

#### 2.1.6 Union

Individual polygons can be unioned (combined) to one multipolygon with `sf::st_union()`. Adding the `Date = ...` argument within the `dplyr::summarise()` function creates a date parameter in the new `smoke_sf_density` data set according to the year, month, and day of the data (defined in step 2.0 Access, download, and. unzip).
...
**Inspecting the structure of `smoke_sf_density` shows three multipolygons of class `sf` with the date.**
...
```{r, include=FALSE}
################################################################################
states_sf = st_read("/Volumes/SET/Projects/EnviroTools/input/states/cb_2018_us_state_500k.shp")
################################################################################
```
...
```{r}
conus_sf = st_transform(conus_sf,
                        5070)
```
...
```{r, warning=FALSE}
smoke_sf_crop = st_crop(smoke_sf_density,
                        conus_sf)
```
...
This concludes section 2.1 Polygon Data with `sf`. Unit 2 of this vignette series will explore more specific analyses that can be conducted with `sf` polygon data.
...
:::callout-note
**Certain R packages have functions with the same, or similar, names but have very different functionality (ie. `stats::aggregate()` and `terra::aggregate()`). When using functions that have common names across different packages, the package needs to be specified (example: `package::function()`).**
:::

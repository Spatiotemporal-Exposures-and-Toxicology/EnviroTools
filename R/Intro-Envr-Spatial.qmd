...
| Land Surface Temperature            | National Aeronautics and Space Administration (NASA)   | <https://lpdaac.usgs.gov/products/mod11a1v006/>                                             |
...
# 
#   smoke_plots <- append(smoke_plots,
#                         smoke_plot_x)
#   
# }
# 
# # create 1 x 3 image with all three smoke density plots
# smoke_arrange <- ggarrange(plotlist=smoke_plots,
#                            nrow=1,
#                            ncol=3)
# 
# smoke_arrange$`1`
...
url_narr <- paste0("https://downloads.psl.noaa.gov//Datasets/NARR/Dailies/monolevel/air.2m.",
                   year,
                   ".nc")
destination_narr <- paste0("/Volumes/SET/Projects/EnviroTools/EnviroTools/input/narr_air2m_",
                           year,
                           ".nc")
download.file(url_narr,
              destination_narr)
...
```{r, echo=FALSE, warning=TRUE, cache=FALSE, results='hide'}
narr <- rast(paste0("/Volumes/SET/Projects/EnviroTools/EnviroTools/input/narr_air2m_",
                    year,
                    ".nc"))
```
...
Re-plot with degrees Celsius.

```{r}
ggplot()+
  geom_spatraster(data=narr_crop_c$air_1)+
  scale_fill_continuous(type="viridis",
                        na.value=NA,
                        "Temperature (°C)")+
  ggtitle("Air Temperature at 2m Height (January 1, 2021)")+
...
narr_crop_c$mean <- mean(narr_crop_c,
                         na.rm=TRUE)
narr_crop_c$range <- max(narr_crop_c,
                         na.rm=TRUE) - min(narr_crop_c,
                                           na.rm = TRUE)
